...
local function wrapped(str, limit, cols)
  ...
  for word in tostring(str):gmatch("%S+") do
    ...
    if #try <= cols then
      ...
      -- Carried in its own variable rather than assigned back into the loop's
      -- own: LuaJIT allows that, Lua 5.4 refuses it, and the difference is
      -- worth nothing here.
      local rest = word
      while #rest > cols do
        push(rest:sub(1, cols))
        rest = rest:sub(cols + 1)
      end
      line = rest
    end
